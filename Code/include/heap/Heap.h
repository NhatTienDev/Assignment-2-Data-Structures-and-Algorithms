...
void Heap<T>::remove(T item, void (*removeItemData)(T))
{
    ...
    elements[foundIndex] = elements[this -> count - 1];
    this -> count--;
...
void Heap<T>::reheapDown(int position)
{
    ...
    if(leftChild < this -> count && aLTb(elements[leftChild], elements[largest]))
    {
        largest = leftChild;
    }

    if(rightChild < this -> count && aLTb(elements[rightChild], elements[largest]))
    {
        largest = rightChild;
    }
